...
namespace {
constexpr double CRIT_CHANCE = 0.10;
constexpr double CRIT_MULTIPLIER = 1.5;
constexpr double ESCAPE_CHANCE = 0.50;
constexpr int BATTLE_LOG_CAP = 20;
} // anonymous namespace
...
void BattleService::attack() {
  if (state_ != BattleState::PlayerTurn)
    return;
  ...
  std::string msg =
      "You attack " + enemy_->name + " for " + std::to_string(dmg) + " damage!";
  if (crit)
...
void BattleService::tryEscape() {
  ...
  if (enemy_->type == MonsterType::Boss) {
    addLog("Cannot escape from a Boss battle!");
    return;
  }
