...
// ShopScreen
// ═══════════════════════════════════════════════════════════════════════════════
ShopScreen::ShopScreen(ShopService *svc, WeaponRegistry *w, ItemRegistry *it,
                       Player *p)
...
bool ShopScreen::update(Key key, char ch) {
  ...
  if (key >= Key::K1 && key <= Key::K9) {
    int num = static_cast<int>(key) - static_cast<int>(Key::K1);
    if (mode_ == Mode::Main && num == 3)
      return false;
    handleSelect();
  }
  return true;
...
void ShopScreen::handleSelect() {
  ...
  if (mode_ == Mode::BuyWeapons) {
    handleBuyWeapon();
    return;
  }
  if (mode_ == Mode::BuyItems) {
    handleBuyItem();
    return;
  }
  if (mode_ == Mode::Sell) {
...
void ShopScreen::handleBuyWeapon() {
  ...
  if (cursor_ < 0 || cursor_ >= (int)entries.size())
    return;
  ...
  if (rc == ResultCode::Success) {
    msg_ = "Purchased " + e.weapon->name + "!";
    msgStyle_ = "success";
  } else if (rc == ResultCode::NotEnoughGold) {
    msg_ = "Not enough gold!";
    msgStyle_ = "error";
  } else if (rc == ResultCode::InventoryFull) {
    msg_ = "Inventory full!";
    msgStyle_ = "error";
  } else {
    msg_ = "Purchase failed!";
    msgStyle_ = "error";
  }
}
...
std::string ShopScreen::render() const {
  ...
  if (mode_ == Mode::Main) {
    out += "\n" + tui::dim("--- Shop Menu ---") + "\n";
    const char *opts[] = {"1. Buy Weapons", "2. Buy Items", "3. Sell Items",
                          "4. Exit Shop"};
    for (int i = 0; i < 4; ++i)
    ...
    for (int i = 0; i < (int)entries.size(); ++i) {
      ...
      if (e.isSoldOut()) {
        ...
        const char *pc =
            player_->gold >= e.weapon->buyPrice ? CLR_GOLD : CLR_DANGER;
        line += " - " + std::string(pc) + std::to_string(e.weapon->buyPrice) +
                " G" + ANSI_RESET;
        line += std::string(CLR_DIM) + " | DMG:" + ANSI_RESET +
                std::to_string(e.weapon->baseDamage);
    ...
    for (int i = 0; i < (int)entries.size(); ++i) {
      ...
      if (e.isSoldOut()) {
        ...
        const char *pc =
            player_->gold >= e.item->buyPrice ? CLR_GOLD : CLR_DANGER;
        line += " - " + std::string(pc) + std::to_string(e.item->buyPrice) +
                " G" + ANSI_RESET;
        line += " " + std::string(CLR_DIM) + "(" + e.item->description + ")" +
                ANSI_RESET;
        if (!e.isUnlimited())
          line += " [Stock:" + std::to_string(e.stock) + "]";
      }
      ...
      for (int i = 0; i < (int)slots.size(); ++i) {
        ...
        if (s->type == SlotType::WeaponSlot && s->weapon) {
          ...
          line += s->item->name + " (x" + std::to_string(s->quantity) + ") - " +
                  Terminal::gold("Sell: " + std::to_string(s->item->sellPrice) +
                                 " G");
        }
        out += (i == cur ? Terminal::selected(line) : line) + "\n";
      }
