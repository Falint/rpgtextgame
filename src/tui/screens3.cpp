...
// InventoryScreen
// ═══════════════════════════════════════════════════════════════════════════════
InventoryScreen::InventoryScreen(Player *p, ItemService *svc)
    : player_(p), svc_(svc) {}
...
void InventoryScreen::handleSelect() {
  if (mode_ == Mode::Main) {
    ...
    case 0:
      ...
      break;
    case 1:
      mode_ = Mode::Consumables;
      cursor_ = 0;
      break;
    case 2:
      mode_ = Mode::Materials;
      cursor_ = 0;
      break;
    case 3:
      mode_ = Mode::UseItem;
...
void InventoryScreen::handleUseItem() {
  ...
  if (slotIdx != -1)
    player_->inventory->removeItem(slotIdx, 1);
  msg_ = result.message;
  msgStyle_ = "success";
}
...
std::string InventoryScreen::render() const {
  ...
  if (mode_ == Mode::Main) {
    out += "Total: " + std::to_string(player_->inventory->usedSlots()) + "/" +
           std::to_string(MAX_INVENTORY_SLOTS) + "\n\n";
    const char *opts[] = {"1. View Weapons", "2. View Consumables",
                          "3. View Materials", "4. Use Item", "5. Back"};
    for (int i = 0; i < 5; ++i)
      out += (i == cursor_ ? Terminal::selected(opts[i])
                           : Terminal::normal(opts[i])) +
             "\n";
    out += "Choice: ";
    ...
    if (ws.empty())
      out += Terminal::muted("  No weapons\n");
    else
      for (int i = 0; i < (int)ws.size(); ++i)
        out += std::to_string(i + 1) + ". " + ws[i]->displayName() + "\n";
    out += "\n" + Terminal::muted("[Esc] Back");
  } else if (mode_ == Mode::Consumables) {
    out += tui::header("--- CONSUMABLES ---") + "\n";
    auto cs = player_->inventory->getConsumables();
    if (cs.empty())
      out += Terminal::muted("  No consumables\n");
    else
      for (int i = 0; i < (int)cs.size(); ++i)
        out += std::to_string(i + 1) + ". " + cs[i]->item->name + " x" +
               std::to_string(cs[i]->quantity) + "\n";
    out += "\n" + Terminal::muted("[Esc] Back");
  } else if (mode_ == Mode::Materials) {
    out += tui::header("--- MATERIALS ---") + "\n";
    auto ms = player_->inventory->getMaterials();
    if (ms.empty())
      out += Terminal::muted("  No materials\n");
    else
      for (int i = 0; i < (int)ms.size(); ++i)
        out += std::to_string(i + 1) + ". " + ms[i].name + " x" +
               std::to_string(ms[i].quantity) + "\n";
    out += "\n" + Terminal::muted("[Esc] Back");
  } else if (mode_ == Mode::UseItem) {
    out += tui::header("--- USE ITEM ---") + "\n";
    auto cs = player_->inventory->getConsumables();
    if (cs.empty()) {
      out += Terminal::muted("  No items to use\n");
    } else {
      int cur3 = tui::clamped(cursor_, (int)cs.size());
      for (int i = 0; i < (int)cs.size(); ++i) {
        std::string line = std::to_string(i + 1) + ". " + cs[i]->item->name +
                           " x" + std::to_string(cs[i]->quantity);
  ...
  if (!msg_.empty())
    out += "\n" + (msgStyle_ == "success" ? Terminal::success(msg_)
                                          : Terminal::error(msg_));
  return out;
}
...
std::vector<EquipScreen::Entry> EquipScreen::getEquippable() const {
  ...
  for (int i = 0; i < MAX_INVENTORY_SLOTS; ++i) {
    auto *s = player_->inventory->getSlot(i);
    if (s && s->type == SlotType::WeaponSlot)
      v.push_back({s->weapon, i});
  }
  return v;
}
void EquipScreen::handleEquip() {
  ...
  if (player_->equippedWeapon)
    player_->inventory->addWeapon(player_->equippedWeapon);
  ResultCode rc;
  auto w = player_->inventory->removeWeapon(ws[cursor_].slotIdx, rc);
  if (rc == ResultCode::Success) {
    player_->equippedWeapon = w;
    msg_ = "Equipped " + w->name + "!";
    msgStyle_ = "success";
  } else {
    msg_ = "Failed to equip!";
    msgStyle_ = "error";
  }
}
...
std::string EquipScreen::render() const {
  ...
  if (player_->equippedWeapon)
    out += std::string(CLR_TEXT) + player_->equippedWeapon->displayName() +
           ANSI_RESET +
  ...
  auto ws = getEquippable();
  if (ws.empty())
    out += Terminal::muted("  No weapons in inventory\n");
  else {
    int curE = tui::clamped(cursor_, (int)ws.size());
    for (int i = 0; i < (int)ws.size(); ++i) {
      std::string line =
          std::to_string(i + 1) + ". " + ws[i].weapon->displayName() +
...
// UpgradeScreen
// ═══════════════════════════════════════════════════════════════════════════════
UpgradeScreen::UpgradeScreen(Player *p, WeaponRegistry *w, ItemRegistry *it,
                             UpgradeRegistry *u)
...
std::vector<UpgradeScreen::Entry> UpgradeScreen::getUpgradeable() const {
  ...
  for (int i = 0; i < MAX_INVENTORY_SLOTS; ++i) {
    auto *s = player_->inventory->getSlot(i);
    if (s && s->type == SlotType::WeaponSlot && s->weapon &&
        s->weapon->canUpgrade())
...
bool UpgradeScreen::checkMaterials(const UpgradeRecipe *r) const {
  for (auto &m : r->materials)
    if (player_->inventory->countItem(m.itemID) < m.quantity)
      return false;
  return true;
}
void UpgradeScreen::consumeMaterials(const UpgradeRecipe *r) {
  for (auto &m : r->materials)
    player_->inventory->removeItemByID(m.itemID, m.quantity);
}
void UpgradeScreen::doUpgradeEquipped() {
  if (!player_->equippedWeapon) {
    msg_ = "No weapon equipped.";
    msgStyle_ = "error";
    return;
  }
  ...
  if (!r) {
    msg_ = w->tier >= w->maxTier ? "Max tier reached!" : "No upgrade path.";
    msgStyle_ = "error";
    return;
  }
  if (player_->gold < r->goldCost) {
    msg_ = "Not enough gold! Need " + std::to_string(r->goldCost) + " G";
    msgStyle_ = "error";
    return;
  }
  if (!checkMaterials(r)) {
    msg_ = "Not enough materials!";
    msgStyle_ = "error";
    return;
  }
  player_->gold -= r->goldCost;
  consumeMaterials(r);
  w->tier = r->toTier;
  w->baseDamage += r->damageBonus;
  msg_ = "Upgraded! " + w->name + " +" + std::to_string(w->tier - 1);
  msgStyle_ = "success";
}
void UpgradeScreen::doUpgradeInventory() {
  auto us = getUpgradeable();
  if (cursor_ < 0 || cursor_ >= (int)us.size())
    return;
  auto *w = us[cursor_].weapon;
  auto *r = upgrades_->getRecipe(w->id, w->tier);
  if (!r) {
    msg_ = "No upgrade available.";
    msgStyle_ = "error";
    return;
  }
  if (player_->gold < r->goldCost) {
    msg_ = "Not enough gold!";
    msgStyle_ = "error";
    return;
  }
  if (!checkMaterials(r)) {
...
std::string UpgradeScreen::render() const {
  ...
  if (mode_ == Mode::Main) {
    ...
    if (us.empty())
      out += Terminal::muted("  No upgradeable weapons.\n");
    else
      for (auto &e : us) {
        std::string tag = e.equipped ? "[E] " : std::string();
        auto *r = upgrades_->getRecipe(e.weapon->id, e.weapon->tier);
        std::string status =
            r ? (checkMaterials(r) && player_->gold >= r->goldCost
                     ? "READY"
                     : "missing mats")
              : "max";
        out +=
            "  " + tag + e.weapon->displayName() +
            (e.equipped ? " (EQUIPPED)" : "") +
            (r ? " -> Tier " + std::to_string(r->toTier) + " (" + status + ")\n"
               : "\n");
      }
    out += "\n" + tui::header("--- Upgrade Menu ---") + "\n";
    const char *opts[] = {"1. Upgrade Equipped", "2. Upgrade Inventory",
                          "3. View Details", "4. Back"};
    for (int i = 0; i < 4; ++i)
      out += (i == cursor_ ? Terminal::selected(opts[i])
                           : Terminal::normal(opts[i])) +
             "\n";
    ...
    for (int i = 0; i < (int)us.size(); ++i) {
      ...
      std::string line = std::to_string(i + 1) + ". " +
                         us[i].weapon->displayName() +
                         (us[i].equipped ? " (EQUIPPED)" : "") +
                         (r ? " [" + std::to_string(r->goldCost) + " G]" : "");
      out += (i == curU ? Terminal::selected(line) : line) + "\n";
      if (i == curU && r) {
        out += "  Requirements:\n";
        out += "    Gold: " + std::to_string(r->goldCost) +
               (player_->gold >= r->goldCost ? " [OK]" : " [NEED MORE]") + "\n";
        for (auto &m : r->materials) {
          int have = player_->inventory->countItem(m.itemID);
          std::string name = m.itemID;
          if (auto *it = items_->getByID(m.itemID))
            name = it->name;
          out += "    " + name + ": " + std::to_string(have) + "/" +
    ...
    if (player_->equippedWeapon) {
      ...
      out += "  Tier: " + std::to_string(w->tier) + "/" +
             std::to_string(w->maxTier) + "\n";
      auto *r = upgrades_->getRecipe(w->id, w->tier);
      if (r)
        out += "  Next Tier +" + std::to_string(r->damageBonus) + " DMG\n";
    } else
      out += Terminal::muted("  No weapon equipped\n");
    out += "\n" + Terminal::muted("[Esc] Back");
  }
  if (!msg_.empty())
    out += "\n" + (msgStyle_ == "success" ? Terminal::success(msg_)
...
std::string StatusScreen::render() const {
  ...
  out += "Name: " + p->name + "\n";
  out += "HP: " +
         Terminal::hpColor(p->currentHP, p->maxHP,
                           std::to_string(p->currentHP) + "/" +
  ...
  if (ta > p->baseAttack)
    out += Terminal::success(" (+" + std::to_string(ta - p->baseAttack) + ")");
  out += "\nTotal Defense: " + std::to_string(td);
  if (td > p->baseDef)
    out += Terminal::success(" (+" + std::to_string(td - p->baseDef) + ")");
  ...
  if (p->buffs.empty())
    ...
    for (auto &b : p->buffs)
      out +=
          std::string(b.type == BuffType::Attack ? "Attack Up" : "Defense Up") +
