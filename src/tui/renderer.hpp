...
static constexpr int MENU_RATIO    = 2;
static constexpr int CHAR_RATIO    = 5;
static constexpr int GAME_RATIO    = 5;
static constexpr int HEADER_HEIGHT = 1;
...
public:
    ...
    void draw(const std::string& menuContent,
...
private:
    ...
    std::vector<std::string> buildPanel(const std::string& content,
...
// ─── Menu panel renderer ──────────────────────────────────────────────────────
struct MenuItem { std::string id, label, key; };
