...
// ─── Combat constants ────────────────────────────────────────────────────────
inline constexpr double DEFENSE_REDUCTION_RATIO = 0.5;
inline constexpr int DEFAULT_BASE_ATTACK = 10;
inline constexpr int MIN_DAMAGE = 1;
...
public:
  explicit Player(const std::string &name,
                  const PlayerConfig &cfg = defaultPlayerConfig());
